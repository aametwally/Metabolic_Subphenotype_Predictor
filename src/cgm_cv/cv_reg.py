...
logfile=resdir+datetime_stmp+"_iterations_"+str(num_iterations)+"_ML_regression_Performance_cv.csv"
f=open(logfile,"a")
f.write("Features,Extraction,Iteration,Fold,Regressor,hyperparameter,RRMSE,Correlation,Accuracy,Precision,Recall,F1"+"\n")
f.close()
# 
ycol=["sspg"]
            ...
            for train_index,validaiton_index in skf.split(X_train[ind_p,],ygroups[ind_p]):
                ...
                if extrmeth=="timeseries":
                    ...
                    for setele in ["train","validation"]:
                        ...
                        locdf=locdf.drop(['subject_id','sspg','muscle_ir_status'],axis=1)
                ...
                for index,(name,regressor) in enumerate(regressors.items()):
                    ...
                    ### RMSE and R2
                    corr=np.corrcoef(y["validation"],y_pred["validation"])[0,1]
                    maxscal=np.concatenate((y["validation"],y_pred["validation"]),axis=None).max()
                    rrmse=metrics.mean_squared_error(y["validation"]/maxscal,y_pred["validation"]/maxscal)**0.5
                    accuracy=metrics.accuracy_score(y["validation"]>IR_thres,y_pred["validation"]>IR_thres)
                    y_validation_clas=y["validation"]>IR_thres
                    y_pred_validation_clas=y_pred["validation"]>IR_thres
                    recallflag=len(np.unique(y_validation_clas))==1
                    precflag=len(np.unique(y_pred_validation_clas))==1
                    if recallflag:
                        recall=float("nan")
                        classifier_auc=float("nan")
                    else:
                        recall=metrics.recall_score(y_validation_clas,y_pred_validation_clas)
                    if precflag:
                        precision=float("nan")
                    else:
                        precision=metrics.precision_score(y_validation_clas,y_pred_validation_clas)
                    if recallflag or precflag:
                        f1=float("nan")
                    else:
                        f1=metrics.f1_score(y_validation_clas,y_pred_validation_clas)
                    ## Logging evaluation results
                    f = open(logfile,"a")
                    f.write(modelset + "," + extrmeth + "_" + pcm + "," + str(iteri) + "," + str(fold) + "," + name + "," + hyperpara_str + "," + str(round(rrmse,2)) + ","+ str(round(corr,2)) +","+str(round(accuracy,2)) +","+ str(round(precision,2)) +","+ str(round(recall,2)) +","+ str(round(f1,2)) +"\n")
                    f.close()
                #
                fold += 1
