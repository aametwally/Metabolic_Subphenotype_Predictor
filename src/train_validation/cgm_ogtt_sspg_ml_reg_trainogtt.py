...
f=open(logfile,"a")
f.write("Features,Extraction,Iteration,Fold,Regresser,RRMSE_validation,RRMSE,Correlation,Accuracy,Precision,Recall,F1"+"\n")
f.close()
# 
ycol=["SSPG"]
        ...
        for iteri in range(num_iterations):
            ...
            for train_index,validaiton_index in skf.split(X_train[ind_p,],ygroups[ind_p]):
                ...
                for index,(name,regresser) in enumerate(regressers.items()):
                    ...
                    # x_trans
                    regresser.fit(x_trans["train"],y["train"])
                    y_pred=dict()
                    y_pred["train"]=regresser.predict(x_trans["train"])
                    y_pred["validation"]=regresser.predict(x_trans["validation"])
                    y_pred["test"]=regresser.predict(x_trans["test"])
                    ### RMSE and R2
                    ...
                    f = open(logfile,"a")
                    f.write(modelset + "," + extrmeth + "_" + pcm + "," + str(iteri) + "," + str(fold) + "," + name + "," + str(round(rrmse_validation,2)) + "," + str(round(rrmse,2)) + ","+ str(round(corr,2)) + ","+ str(round(accuracy,2)) + ","+ str(round(precision,2)) + ","+ str(round(recall,2)) + ","+ str(round(f1,2)) +"\n")
                    f.close()
