...
setwd(resdir)
# 
restab=read.table(paste0("2023-01-29_09-48-50_iterations_20_ML_regression_Performance.csv"),sep=",",header=TRUE)
# name replace
...
for(perfterm in perflist){
    ...
    if(perfterm=="RRMSE"){
        cdata_seletced<-cdata %>% group_by(Features,Extraction) %>% slice(which.min(mean))
    }else{
    ...
    ggsave(paste0("ML_performance_auc_best_all_validation_regress",perfterm,".pdf"),h=10,w=11,plot=p)
}
...
cdata_seletced_valid<- cdata %>% group_by(Features,Extraction) %>% slice(which.min(mean))
...
pdf("method.best.regressor.validation.pdf")
